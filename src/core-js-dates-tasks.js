...
function getDayName(date) {
  const data = new Date(date);
  return data.toLocaleString('en-US', { weekday: 'long' });
}
...
function getNextFriday(date) {
  const data = new Date(date);
  const days = data.getDay() >= 5 ? 12 - data.getDay() : 5 - data.getDay();
...
function isDateInPeriod(date, period) {
  ...
  const startDate = new Date(Object.values(period)[0]);
  const endDate = new Date(Object.values(period)[1]);
...
/**
 * Returns the total number of weekend days (Saturdays aяnd Sundays) in a specified month and year.
 *
...
function getWeekNumberByDate(date) {
  ...
  const startDayOfYear = new Date(`${newDate.getFullYear()}-01-01`);
  const currentDaysOfYear = (newDate - startDayOfYear) / 86400000;
  const offset = startDayOfYear.getDay() === 0 ? 1 : 0;
  return Math.ceil((currentDaysOfYear + startDayOfYear.getDay()) / 7) + offset;
}
...
function getNextFridayThe13th(date) {
  ...
  let resultDate = new Date(year, month, 13);
  while (resultDate.getDay() !== 5) {
    ...
    resultDate = new Date(year, month, 13);
  }
  return resultDate;
}
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  while (currentDate <= endDate) {
    for (let i = 0; i < countWorkDays && currentDate <= endDate; i += 1) {
      schedule.push(getFormatDate(currentDate));
